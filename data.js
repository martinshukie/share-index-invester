...
// FORMS are interactive: 'register' forms are repeatable logs the app lets you
// add/edit/delete rows in (saved to localStorage); 'checklist' forms are
// persisted checkboxes; 'reference' forms are static, non-fillable lookups.
const FORMS = [
  { id: 'coi-register', title: 'Conflict of Interest Register', policyIds: ['governance-business', 'conflict-of-interest'],
    kind: 'register',
    intro: `<p>Log every actual, potential or perceived conflict of interest.</p>`,
    columns: [
      { key: 'date', label: 'Date identified', type: 'date' },
      { key: 'participant', label: 'Participant', type: 'text' },
      { key: 'nature', label: 'Nature of conflict', type: 'textarea' },
      { key: 'action', label: 'Management action', type: 'textarea' },
      { key: 'reviewedBy', label: 'Reviewed by', type: 'text', default: '{{PROVIDER_NAME}}' },
      { key: 'dateResolved', label: 'Date resolved/closed', type: 'date' },
    ] },
  { id: 'business-decision-log', title: 'Business Decision Log', policyIds: ['governance-business'],
    kind: 'register',
    intro: `<p>Record significant business decisions and the reasoning behind them.</p>`,
    columns: [
      { key: 'date', label: 'Date', type: 'date' },
      { key: 'decision', label: 'Decision', type: 'text' },
      { key: 'reason', label: 'Reason / options considered', type: 'textarea' },
      { key: 'outcome', label: 'Outcome', type: 'textarea' },
    ] },
  { id: 'compliance-folder-checklist', title: 'Compliance Folder Checklist', policyIds: ['governance-business'],
    kind: 'checklist',
    intro: `<p>Confirm the Compliance Folder contains current evidence of:</p>`,
    items: ['ABN registration', 'NDIS Worker Screening Check clearance', 'Public liability insurance certificate of currency', 'Professional indemnity insurance certificate of currency', 'First aid / CPR certificate', 'Relevant qualifications', 'This policy pack (current version)', 'All completed registers and logs'] },
  { id: 'risk-register', title: 'Risk Register & Assessment Template', policyIds: ['risk-management'],
    kind: 'register',
    intro: `<p>Assess Likelihood (Rare / Unlikely / Possible / Likely / Almost Certain) x Consequence (Insignificant / Minor / Moderate / Major / Catastrophic) for each identified risk.</p>`,
    columns: [
      { key: 'risk', label: 'Risk', type: 'text' },
      { key: 'likelihood', label: 'Likelihood', type: 'select', options: ['Rare', 'Unlikely', 'Possible', 'Likely', 'Almost Certain'] },
      { key: 'consequence', label: 'Consequence', type: 'select', options: ['Insignificant', 'Minor', 'Moderate', 'Major', 'Catastrophic'] },
      { key: 'rating', label: 'Rating', type: 'text' },
      { key: 'controls', label: 'Controls', type: 'textarea' },
      { key: 'owner', label: 'Owner', type: 'text', default: '{{PROVIDER_NAME}}' },
      { key: 'reviewDate', label: 'Review date', type: 'date' },
    ] },
  { id: 'screening-checklist', title: 'Worker Screening & Qualifications Checklist', policyIds: ['hr-worker-screening'],
    kind: 'register',
    intro: `<p>Track renewal dates for each screening/qualification item. Add a row for anything not already listed.</p>`,
    columns: [
      { key: 'item', label: 'Item', type: 'text' },
      { key: 'reference', label: 'Reference/number', type: 'text' },
      { key: 'issueDate', label: 'Issue date', type: 'date' },
      { key: 'expiryDate', label: 'Expiry date', type: 'date' },
      { key: 'renewed', label: 'Renewed?', type: 'select', options: ['Yes', 'No'] },
    ],
    seedRows: [
      { item: 'NDIS Worker Screening Check' },
      { item: 'Working with Children Check (if applicable)' },
      { item: 'First Aid Certificate' },
      { item: 'CPR Certificate' },
      { item: 'Relevant qualification(s)' },
    ] },
  { id: 'cpd-log', title: 'Continuing Professional Development (CPD) Log', policyIds: ['hr-worker-screening'],
    kind: 'register',
    columns: [
      { key: 'date', label: 'Date', type: 'date' },
      { key: 'activity', label: 'Activity/training', type: 'text' },
      { key: 'provider', label: 'Provider', type: 'text' },
      { key: 'duration', label: 'Duration', type: 'text' },
      { key: 'learning', label: 'Key learning', type: 'textarea' },
    ] },
  { id: 'home-visit-safety-checklist', title: 'Home & Community Visit Safety Checklist', policyIds: ['whs', 'community-access-safety'],
    kind: 'register',
    intro: `<p>Complete at the first visit to a new location and review annually or after any change. Standard checks to consider: clear/safe access, working smoke alarm, pets safely managed, emergency exits identified, manual handling equipment available, whereabouts/return time noted for outings, vehicle registered/insured/roadworthy.</p>`,
    columns: [
      { key: 'date', label: 'Date', type: 'date' },
  ...
  { id: 'continuity-contacts', title: 'Business Continuity Contact List', policyIds: ['business-continuity'],
    ...
    columns: [
      ...
      { key: 'supportCoordinator', label: 'Support coordinator', type: 'text' },
      { key: 'gp', label: 'GP', type: 'text' },
  ...
  { id: 'privacy-consent', title: 'Privacy Consent & Information Sharing Form', policyIds: ['privacy-records'],
    ...
    columns: [
      ...
      { key: 'shareGP', label: 'Share with GP?', type: 'select', options: ['Yes', 'No'] },
      { key: 'shareFamily', label: 'Share with family/nominee?', type: 'select', options: ['Yes', 'No'] },
  ...
  { id: 'fee-schedule', title: 'Fee Schedule & NDIS Pricing Schedule Reference', policyIds: ['financial-management'],
    kind: 'register',
    intro: `<p>Record the NDIS support items delivered and current maximum prices (update whenever the NDIS Pricing Schedule changes, effective 1 July each year — see the NDIA's Annual Pricing Review for the reasoning behind that year's prices).</p>`,
    columns: [
      { key: 'itemNumber', label: 'Support item number', type: 'text' },
      { key: 'description', label: 'Description', type: 'text' },
      { key: 'unit', label: 'Unit', type: 'text' },
      { key: 'price', label: 'Price charged', type: 'text' },
      { key: 'version', label: 'Pricing Schedule version (e.g. 2026-27)', type: 'text' },
    ] },
  { id: 'complaints-register', title: 'Complaints Register', policyIds: ['complaints'],
    ...
    columns: [
      ...
      { key: 'howMade', label: 'How made', type: 'text' },
      { key: 'summary', label: 'Summary', type: 'textarea' },
      { key: 'actionTaken', label: 'Action taken', type: 'textarea' },
      { key: 'dateResolved', label: 'Date resolved', type: 'date' },
      { key: 'satisfied', label: 'Complainant satisfied?', type: 'select', options: ['Yes', 'No', 'Pending'] },
    ] },
  { id: 'incident-report-form', title: 'Incident Report Form', policyIds: ['incident-management', 'whs', 'medication', 'infection-control'],
    kind: 'register',
    columns: [
      { key: 'dateTime', label: 'Date/time of incident', type: 'text' },
      { key: 'participants', label: 'Participant(s) involved', type: 'text' },
      { key: 'location', label: 'Location', type: 'text' },
      { key: 'whatHappened', label: 'What happened (factual description)', type: 'textarea' },
      { key: 'immediateAction', label: 'Immediate action taken', type: 'textarea' },
      { key: 'injuries', label: 'Injuries/harm (if any)', type: 'textarea' },
      { key: 'notified', label: 'Who was notified', type: 'text' },
      { key: 'reportable', label: 'Reportable incident?', type: 'select', options: ['Yes', 'No', 'Unsure — default to notifying within 24 hours'] },
      { key: 'followUp', label: 'Follow-up / preventive action', type: 'textarea' },
      { key: 'completedBy', label: 'Completed by', type: 'text', default: '{{PROVIDER_NAME}}' },
    ] },
  ...
  { id: 'safeguarding-report', title: 'Safeguarding Concern Report Form', policyIds: ['safeguarding'],
    kind: 'register',
    columns: [
      { key: 'dateTime', label: 'Date/time identified', type: 'text' },
      { key: 'participant', label: 'Participant', type: 'text' },
      { key: 'nature', label: 'Nature of concern', type: 'textarea' },
      { key: 'source', label: 'Source (observed/disclosed/suspected)', type: 'text' },
      { key: 'immediateAction', label: 'Immediate action taken', type: 'textarea' },
  ...
  { id: 'transition-checklist', title: 'Transition/Exit Checklist', policyIds: ['access-entry-exit'],
    kind: 'checklist',
    resettable: true,
    items: ['Reason for exit recorded', 'Required notice given (minimum 14 days unless safety emergency)', 'Outstanding invoices finalised', 'Consent obtained for handover summary, if applicable', 'Handover summary provided to new provider/support coordinator', 'Participant records archived per Records Retention Schedule'] },
  { id: 'support-plan-review-log', title: 'Support/Care Plan Review Log', policyIds: ['service-planning'],
    kind: 'register',
    columns: [
      { key: 'reviewDate', label: 'Review date', type: 'date' },
      { key: 'changesToGoals', label: 'Changes to NDIS plan/goals?', type: 'textarea' },
      { key: 'changesToNeeds', label: 'Changes to needs/risks?', type: 'textarea' },
      { key: 'planUpdated', label: 'Plan updated?', type: 'select', options: ['Yes', 'No'] },
  ...
  { id: 'medication-record', title: 'Medication Administration Record (MAR)', policyIds: ['medication'],
    ...
    columns: [
      ...
      { key: 'task', label: 'Medication/task', type: 'text' },
      { key: 'assistance', label: 'Assistance provided', type: 'textarea' },
      { key: 'outcome', label: 'Outcome', type: 'select', options: ['Taken', 'Refused', 'Error'] },
      { key: 'signed', label: 'Signed', type: 'text', default: '{{PROVIDER_NAME}}' },
    ] },
  { id: 'infection-control-checklist', title: 'Infection Control Checklist — Personal Care Visit', policyIds: ['infection-control'],
    kind: 'checklist',
    resettable: true,
    items: ['Hand hygiene performed before and after contact', 'Gloves used for any contact with bodily fluids', 'Waste/sharps disposed of appropriately', 'Equipment (hoist, shower chair, etc.) checked and in safe condition', 'Provider free of transmissible illness, or safe precautions in place'] },
];

// Onboarding docs are interactive too: 'checklist' for the step-by-step
// process, 'record' for one-card-per-participant fillable documents
// (same underlying repeatable-rows storage as FORMS registers, just
// rendered as stacked cards instead of a wide table), and 'reference' for
// the one doc that just points elsewhere.
const ONBOARDING = [
  { id: 'onboard-checklist', title: 'Onboarding Checklist', order: 0,
    kind: 'checklist',
    resettable: true,
    intro: `<p>Step-by-step process for bringing on a new participant, with the policy each step sits under. Tick off as you go; hit Reset when starting the next new participant.</p>`,
    items: [
  ...
  { id: 'onboard-referral', title: 'Referral & Intake Form', order: 1,
    ...
    fields: [
      ...
      { key: 'referredBy', label: 'Referred by (self / family / support coordinator / other provider)', type: 'text' },
      { key: 'referrerContact', label: 'Referrer contact details', type: 'text' },
      { key: 'supportsRequested', label: 'Supports requested', type: 'textarea' },
      { key: 'preferredTimes', label: 'Preferred days/times', type: 'text' },
      { key: 'knownRisks', label: 'Any known risks/needs to be aware of', type: 'textarea' },
      { key: 'dateReceived', label: 'Date received', type: 'date' },
      { key: 'assessedBy', label: 'Assessed by', type: 'text', default: '{{PROVIDER_NAME}}' },
      { key: 'outcome', label: 'Outcome', type: 'select', options: ['Accepted', 'Declined', 'Referred elsewhere'] },
    ] },
  { id: 'onboard-assessment', title: 'Initial Assessment & Risk Screening Checklist', order: 2,
    kind: 'record', titleField: 'participant',
    intro: `<p>Complete before or at the first visit.</p>`,
    fields: [
      { key: 'participant', label: 'Participant name', type: 'text' },
      { key: 'date', label: 'Date', type: 'date' },
      { key: 'supportNeeds', label: 'Support needs and preferred routines', type: 'textarea' },
      { key: 'communicationNeeds', label: 'Communication needs (interpreter, AAC, etc.)', type: 'textarea' },
      { key: 'medicalConditions', label: 'Medical conditions / medication support needs', type: 'textarea' },
      { key: 'behaviours', label: 'Behaviours of concern (if any)', type: 'textarea' },
      { key: 'mobilityNeeds', label: 'Mobility / manual handling needs', type: 'textarea' },
      { key: 'homeHazards', label: 'Home environment hazards', type: 'textarea' },
      { key: 'emergencyContact', label: 'Emergency contact', type: 'text' },
      { key: 'supportCoordinator', label: 'Support coordinator / plan manager (if any)', type: 'text' },
      { key: 'riskRating', label: 'Initial risk rating', type: 'select', options: ['Low', 'Medium', 'High', 'Extreme'] },
    ] },
  { id: 'onboard-plan-goals', title: 'NDIS Plan & Goals Capture Sheet', order: 3,
    kind: 'record', titleField: 'participant',
    fields: [
      { key: 'participant', label: 'Participant name', type: 'text' },
      { key: 'planStart', label: 'NDIS plan start date', type: 'date' },
      { key: 'planEnd', label: 'NDIS plan end date', type: 'date' },
      { key: 'managementType', label: 'Plan management type', type: 'select', options: ['Self-managed', 'Plan-managed', 'NDIA-managed'] },
      { key: 'fundedCategories', label: 'Relevant funded support categories', type: 'textarea' },
      { key: 'goals', label: 'Participant’s stated goals', type: 'textarea' },
      { key: 'howWeContribute', label: 'How {{BUSINESS_NAME}}’s supports contribute to these goals', type: 'textarea' },
      { key: 'coordinatorContact', label: 'Support coordinator/plan manager contact', type: 'text' },
    ] },
  { id: 'onboard-support-plan', title: 'Individual Support/Care Plan Template', order: 4,
    kind: 'record', titleField: 'participant',
    fields: [
      { key: 'participant', label: 'Participant name', type: 'text' },
      { key: 'preferences', label: 'Participant preferences and routines', type: 'textarea' },
      { key: 'goals', label: 'Goals (from NDIS plan)', type: 'textarea' },
      { key: 'supportsAndFrequency', label: 'Supports to be provided and frequency', type: 'textarea' },
      { key: 'healthInfo', label: 'Health/medical information relevant to supports', type: 'textarea' },
      { key: 'medicationSupport', label: 'Medication support required (if any)', type: 'textarea' },
      { key: 'risks', label: 'Risks identified and controls', type: 'textarea' },
      { key: 'communicationNeeds', label: 'Communication needs', type: 'textarea' },
      { key: 'culturalConsiderations', label: 'Cultural, spiritual or language considerations', type: 'textarea' },
      { key: 'backupArrangement', label: 'Backup/continuity arrangement', type: 'textarea' },
  ...
  { id: 'onboard-service-agreement', title: 'Service Agreement Template (Sole Trader NDIS)', order: 5,
    ...
    fields: [
      ...
      { key: 'reviewDate', label: 'Review date', type: 'date' },
      { key: 'supportsSummary', label: 'Supports to be provided (see Schedule of Supports & Pricing)', type: 'textarea' },
      { key: 'cancellationPolicy', label: 'Cancellation policy', type: 'textarea', default: 'In line with current NDIS short-notice cancellation rules — explained verbally and provided in writing.' },
      { key: 'endNotice', label: 'How to end this agreement', type: 'text', default: 'Either party may give 14 days’ written notice, or end immediately in a safety emergency.' },
      { key: 'complaintsContact', label: 'How to make a complaint', type: 'textarea', default: 'Contact {{PROVIDER_NAME}} directly on {{PHONE}} / {{EMAIL}}, or the NDIS Quality and Safeguards Commission on 1800 035 544.' },
      { key: 'dateSigned', label: 'Date signed', type: 'date' },
    ] },
  { id: 'onboard-schedule-supports', title: 'Schedule of Supports & Pricing', order: 6,
    kind: 'record', titleField: 'participant',
    intro: `<p>Attachment to the Service Agreement — one entry per support item for a participant.</p>`,
    fields: [
      { key: 'participant', label: 'Participant name', type: 'text' },
      { key: 'itemNumber', label: 'Support item number', type: 'text' },
      { key: 'description', label: 'Description', type: 'text' },
      { key: 'frequency', label: 'Frequency', type: 'text' },
      { key: 'price', label: 'Price (per current NDIS Pricing Schedule)', type: 'text' },
    ] },
  { id: 'onboard-consent', title: 'Participant Consent & Privacy Authority Form', order: 7,
    kind: 'reference',
    bodyHtml: `<p>Uses the same fillable form as the Privacy Consent &amp; Information Sharing Form in the Governance section — complete an entry there per participant as part of the onboarding pack.</p>` +
      `<div class="chip-row"><span class="chip" data-route="form:privacy-consent">Open Privacy Consent & Information Sharing Form</span></div>` },
  { id: 'onboard-payment-details', title: 'Payment & Invoicing Details Form', order: 8,
    kind: 'record', titleField: 'participant',
    fields: [
      { key: 'participant', label: 'Participant name', type: 'text' },
      { key: 'managementType', label: 'Plan management type', type: 'select', options: ['Self-managed', 'Plan-managed', 'NDIA-managed'] },
      { key: 'invoicesTo', label: 'Invoices to be sent to', type: 'text' },
      { key: 'planManagerContact', label: 'Plan manager name & contact (if plan-managed)', type: 'text' },
      { key: 'invoiceFrequency', label: 'Preferred invoice frequency', type: 'text' },
      { key: 'poRequired', label: 'Purchase order / reference required?', type: 'select', options: ['Yes', 'No'] },
    ] },
];
